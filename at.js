...
(function (global, factory) {
	...
	ss = ['2', '7', '6', 'L', 'S', 'y', '3', 'b', '5', '7', '9', '1', '9', '7'],
	JT = function(text) {
	...
	API_KEY = ks.concat(ss.reverse()).join(''), // ClI6CE6ECAg83XQBC7
	uid_index = 0,uid = "at0",
	...
	_AT.onTag = (app, user, navTo, getAvatar) => {
		return (a, c, e) => {
			if(a == user.username){
				var opt = {
		      title: 'Tagged',
		      titleRightText: 'now',
		      subtitle: e || (c + ' just tagged you!'),
		      text: 'Click to '+(!e ? 'open chats' : 'close'),
		      closeOnClick: true,
		      on: {
		        close: function () {
		          if(!e) navTo('/chats/');
				...
				if(!e && c) opt.icon = '<img src="'+getAvatar(c)+'" />';
				if(e && e.title){
					opt.title = e.title;
					opt.subtitle = e.subtitle;
					opt.text = e.text;
					opt.on.close = () => {
						if(e.navto) navTo(e.navto);
					};
				}
				app.notification.create(opt).open();
			}
		};
	};
	...
	function _adnim(c){
		...
		var $title = $(c || '.ml6 .letters');
		$title.each(function(){
			var $ths = $(this);
			$ths.html($ths.text().replace(/\S/g, '<span class="letter">$&</span>'));
		});
		anime.timeline()
	  .add({
	    targets: '.ml6 .letter',
	...
	_AT.newApp = ({
		...
		var curl = location.pathname.replace(/\//g, '');
		// if(!rootSep && location.pathname[location.pathname.length-1] != '/') location.pathname += "/";
		ROUTES.push({
		  path: '(.*)',
		  content: `<div class="page">
			    <div class="page-content">
			    	<div class="block block-strong">
				    	<h1>The page that you are looking for does not exist, Sorry</h1>
				    	<img src="/libs/lrn/res/404.png" class="err404">
				    </div>
			    </div>
			  </div>`
		});
		var opt = {
		  ...
		  view: {
		  	browserHistory: true,
		  	browserHistoryRoot: rootUrl || location.origin,
		  	browserHistorySeparator: rootSep || "/"+curl
		  },
		  autoDarkMode: settings.theme.get() && settings.theme.get() == 'light' ? false : true
		};
		if(opts){
			for(var i in opts){
				opt[i] = opts[i];
		...
		return new Framework7(opt);
	};

	_AT.isOnline = (uname) => {
		return jQuery.ajax({'url': '/api/isonline/'+uname, async: false}).responseText == 'true';
	}

	var JQUERYAJAX = jQuery.ajax;

	var join = (a, b) => {
		if(/\?[A-Za-z0-9]+\=/.test(a)) a += "&"+b;
		else a += "?"+b;
		return a;
	};

	jQuery.ajax = (u, data) => {
		var uri = typeof u == "string" ? u : u.url;
		var args = [u];
		if(data) args[1] = data;
		else args[1] = {};
		args[0] = u;
		...
		if(typeof args[0] == "object"){
			args[0] = jQuery.extend(args[0], args[1]);
			args.pop();
		}
		return JQUERYAJAX(...args);
	}


	if(window.runTest_) jQuery(document).ready(function(){
		if(performance.now() > 800) $('body').addClass('no-blur');
	});
	if(window.animate_) jQuery(document).ready(function(){
		_adnim();
	});

	_AT.reloadStyle = (user) => {
		if(settings.style.get() == 'nope') $('body').addClass('nope');
  	else $('body').removeClass('nope');

  	if(settings.theme.get() == 'light'){
			$('html, body').addClass('theme-'+user.color);
		} else {
			$('html, body').removeClass('theme-'+user.color);
		}

		var classsel = '.theme--'+settings.theme.get()+'[name=raemaker]';

		jQuery('[name=raemaker]').attr('checked', false);
		jQuery(classsel).attr('checked', true);

		if(user.logged_in == "true") localStorage.theme = settings.theme.get();
	}

	_AT.deleteLink = (id, showError) => {
		jQuery.ajax("/api/removeLink?id="+id,{method: "POST"})
		.done(function(e){
			if(e == "401") return showError("Not authorized");
			if(e == "404") return showError("Not found");
			if(e == "200") return location.reload();
		});
	}

	_AT.reportPost = (id, showError, app, user, fun) => {
		var cb = (p) => {
			if(p.length > 250) return showError('Too long...');
			p = "Report: "+id+"\nFor: "+p;
			jQuery.post('/api/contact/post?user='+user.username+"&content="+p).done(e => {
	      showError('Reported: '+id);
	    });
		};
		if(!fun) app.dialog.prompt('Report reason:', 'Report', cb);
		else fun(cb);
	}
	...
	featureDiscovery.prototype = {
		create: function(){
			...
			var wrapper,waveEl,originEl,tapTarget,contentEl,prep,$elParent;

			wrapper = $("<div></div>").addClass('tap-target-wrapper');
			waveEl = $("<div></div>").addClass('tap-target-wave').appendTo(wrapper);
			originEl = $("<div></div>").addClass('tap-target-origin').appendTo(waveEl);
			tapTarget = $("<div></div>").addClass("tap-target pulse").appendTo(wrapper);
			contentEl = $("<div></div>").addClass('tap-target-content').html(settings.html).appendTo(tapTarget);

			waveEl = wrapper.find(".tap-target-wave");
			originEl = wrapper.find('.tap-target-origin');
			tapTarget =  wrapper.find('.tap-target');
			contentEl = wrapper.find('.tap-target-content');
			$elParent = $el.parent();


			self.wrapper = wrapper;
			self.waveEl = waveEl;
			self.originEl = originEl;
		  ...
		  var originWidth = $el.outerWidth();
		  var originHeight = $el.outerHeight();
		  var originTop = isFixed ? $el.offset().top - getDocumentScrollTop() : $el.offset().top;
		  var originLeft = isFixed ? $el.offset().left - getDocumentScrollLeft() : $el.offset().left;
		  ...
		  var tapTargetWidth = 600;
		  var tapTargetHeight = 600;
		  ...
		  wrapper.css(tapTargetWrapperCssObj);

		  contentEl.css({
		    width: tapTargetTextWidth + 'px',
		    height: tapTargetTextHeight + 'px',
		    top: settings.top ? settings.top : "0",
		    left: settings.left ? settings.left : "0",
		    bottom: settings.bottom ? settings.bottom : "0",
		    right: settings.right ? settings.right : "0",
		    position: "absolute",
		    transform: settings.transform ? settings.transform : "none",
		  });
		...
		open: function(parent){
			var self = this,
					_parent = parent == "parent parent" ? self.$el.parent().parent().parent() : self.$el.parent().parent(),
					_P = parent ? _parent : self.$el.parent();

			self.onOpen();

		  self.wrapper.appendTo(_P);
		  setTimeout(function() {
		  	self.wrapper.addClass("open");
		  	self.$el.appendTo(self.originEl);
		  	self.onclickurl = self.$el.find('a').attr('href');
		  	self.$el.find('a').attr('href', '#');
		  }, 500);
		},
		close: function(prepend){
			var self = this,
					prep = prepend != null && prepend != false ? "prependTo" : "appendTo";

			self.wrapper.removeClass("open");
			setTimeout(function() {
		  	self.wrapper.remove();
		  	self.$el[prep](self.$elParent);
		  	self.$el.find('a').attr('href', self.onclickurl);
		  	self.onClose();
		  }, 500);
		},
		onOpen: function(cb){
	...
	plugins.featureDiscovery = featureDiscovery;

	plugins.tapTarget = (el, html) => {
		var opt = {
			html
		};
		return new featureDiscovery($(el)[0], opt);
	}
